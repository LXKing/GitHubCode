...
{
    public partial class frmTreeRecycleBin : FormBase
    {
        public frmTreeRecycleBin()
        {
            InitializeComponent();
        }
        ...
        protected void BindControl()
        {
            ...
            gridControl.LoadConfig(Guid.Empty);
            gridControl.KeyFieldName = Operate.Metadata.PrimaryKeyFieldName;
            gridControl.ParentFieldName = "~";
            gridControl.OptionsBehavior.DragNodes = false;
            gridControl.OptionsView.ShowRoot = false;
            gridControl.OptionsView.ShowCheckBoxes = true;
        ...
        protected void ForeachSelectedList(Action<EntityBase> action)
        {
            ...
            {
                EntityBase item = List.Grid.Get<EntityBase>(list[index]);
                action(item);
            }
        }
        ...
        private TreeListOperate<EntityBase, IEntityService<EntityBase>> _list;
        /// <summary>
        /// 操作对象
        /// </summary>
        public TreeListOperate<EntityBase, IEntityService<EntityBase>> List
        {
            get
            {
                if (_list == null)
                {
                    _list = new TreeListOperate<EntityBase, IEntityService<EntityBase>>();
